...
func minMaxNamed(nums []int) (min, max int) {
	...
	return // 裸 return（naked return）：自动返回 min 和 max 的当前值
}

// 命名返回值的实际用途：配合 defer 修改返回值
// 场景：统一处理 error，在 defer 里给 err 赋值
func riskyOp() (result int, err error) {
	defer func() {
		if err != nil {
			// defer 可以读写命名返回值，在函数真正返回前修改它
...
func main() {
	...
	fmt.Println("sum():", sum()) // 0 个也合法，nums 是空 slice

	// 展开 slice 传入：用 ... 展开，类比 JS 的 spread: sum(...arr)
	arr := []int{1, 2, 3, 4, 5}
